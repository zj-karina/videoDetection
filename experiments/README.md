...
Для обеспечения работы модели на мобильных устройствах с ограниченными вычислительными ресурсами, можно использовать ONNX (Open Neural Network Exchange). 
Это позволяет перенести нашу модель на различные платформы, в том числе на мобильные устройства, с сохранением высокой производительности. При этом время обработки на CPU мобильного устройства будет порядка 100 мс на изображение.
Дополнительно, для улучшения производительности и снижения требований к памяти на мобильных устройствах, можно применить квантизацию модели. 
### Масштабируемость решения:
Для добавления новых типов дорожных знаков в модель, есть возможность её дообучения. Это включает в себя обновление и расширение набора данных с новыми изображениями дорожных знаков и последующее дообучение модели. Важным шагом является модификация последнего слоя (linear layer) для адаптации модели к расширенному набору классов объектов. Можно взять dataset.yaml, добавть название новых классов и их кол-во и после этого снова обучить модель.
